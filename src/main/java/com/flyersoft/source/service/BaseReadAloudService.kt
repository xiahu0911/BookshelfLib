...
import com.flyersoft.source.conf.Status
...
//    private lateinit var mediaSessionCompat: MediaSessionCompat
    ...
    private var title: String = "哈哈哈哈"
    private var subtitle: String = "测试测试"
...
//        mediaSessionCompat = MediaSessionCompat(this, "readAloud")
        httpTTS = HttpTTSController.getInstance().get(1598233029307)
        initMediaSession()
...
//        mediaSessionCompat.release()
    ...
    open fun newReadAloud(dataKey: String?, play: Boolean) {
        dataKey?.let {
            var data = IntentDataHelp.getData<List<String>>(dataKey)
            data?.let { textChapter ->
                nowSpeak = 0
                contentList.clear()
                contentList.addAll(textChapter)
                if (play) play()
            } ?: stopSelf()
        } ?: stopSelf()
    }
